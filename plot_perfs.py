...
if __name__ == "__main__":
    ...
    parser.add_argument('--minN','-miN',dest='min_N',default=0,help='minimum number of nodes')
    parser.add_argument('--maxN','-maN',dest='max_N',default=None,help='maximum number of nodes') 
    parser.add_argument('--xtick','-x',dest='xticks',default=5,help='number of ticks on x axis')  
    parser.add_argument('--tit','-t',dest='title',default='Scaling plot',help='title') 
    parser.add_argument('--name','-n',dest='name_plot',default='scaling_plot',help='name of plot')  
    ...
    if len(experiments) == 0:
        ...
        experiments = [defexp.experiment(name = os.path.basename(fn).split('.csv')[0],marker='o') for fn in csv_files]
    ...
    for var_to_plot in variables :
        ...
        for exp_name in experiments:
            ...
            # plot
            dt.plot(kind='line', x='N_Nodes', y=var_to_plot, ax=ax,label=exp.label, title=args.title+', '+var_to_plot, **exp.line_appareance)
        ...
        ax.set_xticks(np.arange(args.min_N, args.max_N+1, step=args.xticks), minor=False)
        ...
        if lo_write_csv:
            filename_out = os.path.join(path,'summary_{}_tot_{}.csv'.format(var_to_plot,args.name_plot))
