...
if __name__ == "__main__":
    ...
    if (len(args.cpus_to_proceed) > 0) and (len(args.nodes_to_proceed) > 0) :
        print('You can specify either the number of cpus or the number of nodes, not both.')
        print('Exiting')
        exit(1)

    if (len(args.nodes_to_proceed) > 0):
        args.cpus_to_proceed = args.cpu_per_node * np.array(args.nodes_to_proceed) 
        l_cpus_def = True
    ...
    if not l_cpus_def :
        args.cpus_to_proceed = (np.arange(args.nbeg_iter,args.niter) * args.cpus_incr)
        l_cpus_def = True
    ...
    if (exp_name_bas_exp.split('_')[-1].startswith('cpus')):
        exp_name_nucl = '_'.join(exp_name_bas_exp.split('_')[:-1])                # name of the new experiments
    else:
 ...
 #   os.chdir(args.basis_folder)
    ...
    for ncpus in args.cpus_to_proceed:
        ...
        new_exp_name = '%s_cpus%i' %(exp_name_nucl,ncpus)                  # new experiment name
        
        # new walltime
        comp_walltime = (walltime_datetime * ncpus_bas/ncpus)
        new_walltime = datetime.timedelta(seconds=round(comp_walltime.total_seconds()))
        ...
        print ('jobsubm_echam.sh -o "{}" {}'.format(string_to_overwrite,setting_bas_exp))
        print('--------------------------------------------------------------------------------------------------------')
        if not args.d :
            os.system('jobsubm_echam.sh -o "{}" {}'.format(string_to_overwrite,setting_bas_exp))
